...
def get_supervised_data(dataset, ratios, seed, cache_dir,few_shot):
    ...
    if(few_shot == 0):
        ...
        total_len = len(val_test_list)
        val_size = int(ratios[0] * total_len)
        test_size = total_len - val_size

        val_set, test_set = random_split(val_test_list, [val_size, test_size], torch.Generator().manual_seed(seed))
